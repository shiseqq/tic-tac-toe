...
void menu()
{
    ...
    Image ximage;
    ximage.loadFromFile("../bin/i1.png");
...
{
    Image pimage;
    pimage.loadFromFile("../bin/pole.png");
    Texture ptexture;
    ...
    while (window.isOpen()) {
        ...
        if (Mouse::isButtonPressed(Mouse::Left)
            && IntRect(840, 500, 300, 80)
                       .contains(Mouse::getPosition(window))) {
            system("/home/shise/myProj/tic-tac/bin/tic-tac-toe.exe");
            exit(0);
        }
        if (Mouse::isButtonPressed(Mouse::Left)
